		...
		if ( $isNew ) {
			if ( $enableExperimentalCVTFeatures ) {
				...
				if ( $matches || $shouldSendToCVTFeed ) {
					...
					( new $this->discordNotifier )->notify( $message, $user, 'article_inserted', [
			...
			( new $this->discordNotifier )->notify( $message, $user, 'article_inserted', [], null );
		} else {
			...
			if ( $enableExperimentalCVTFeatures ) {
				...
				if ( $matches || $shouldSendToCVTFeed ) {
					...
					( new $this->discordNotifier )->notify( $message, $user, 'article_saved', [
						$this->discordNotifier->getMessage( 'discordnotifications-summary', '' ) => $summary,
			...
			( new $this->discordNotifier )->notify( $message, $user, 'article_saved', [], null );
		}
	}
